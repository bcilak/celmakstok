...
# ÇELMAK Stok Takip Sistemi - Sunucu Yeniden Başlatma ve Güncelleme Betiği
# Bu betik sunucudaki en son değişiklikleri çeker, çalışan eski süreçleri sonlandırır ve uygulamayı yeniler.

echo "=========================================="
echo "🔄 ÇELMAK Stok Takip Uygulaması Güncelleniyor..."
echo "=========================================="
...
if systemctl is-active --quiet celmakstok.service 2>/dev/null; then
    echo "🔄 systemd servisi (celmakstok.service) yeniden başlatılıyor..."
    sudo systemctl restart celmakstok.service
...
# Port 5000'i dinleyen süreci bulup sonlandır
PID=$(lsof -t -i:5000 2>/dev/null)
if [ -n "$PID" ]; then
    echo "Port 5000 üzerinde çalışan süreç bulundu (PID: $PID). Sonlandırılıyor..."
    kill -9 $PID 2>/dev/null
    sleep 2
else
    # Alternatif olarak gunicorn adına göre sonlandır
    pkill -9 -f "gunicorn.*run:app" 2>/dev/null
    sleep 2
fi
...
nohup $VENV_PATH/bin/gunicorn --workers 4 --bind 127.0.0.1:5000 --timeout 120 run:app > gunicorn.log 2>&1 &

# Başlatma kontrolü
sleep 3
NEW_PID=$(lsof -t -i:5000 2>/dev/null)
if [ -n "$NEW_PID" ]; then
    ...
    NEW_GUNICORN_PID=$(pgrep -f "gunicorn.*run:app")
    if [ -n "$NEW_GUNICORN_PID" ]; then
         echo "✅ Uygulama başarıyla başlatıldı ve arka planda çalışyor (PID: $NEW_GUNICORN_PID)."
    else
